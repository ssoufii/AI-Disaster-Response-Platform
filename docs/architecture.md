...
- Every channel in the enum can be sent on as of #19, so a fallback anywhere in a household's chain is dispatched for real — a failed SMS onto `video` sends the clip, exactly as onto `voice` it places the call. A row whose channel somehow could not be sent on would still be left `queued` and logged as `delivery.channel_not_implemented` rather than marked failed: nothing was tried, and calling it a failure would walk the household one step further down its chain for a channel nobody attempted.
...
- All four channels are wired up: `sms`, `voice`, and `video`/`whatsapp` (the ASL clip send, #19). A household on a channel outside the enum would still be logged and left without an attempt rather than sent something it cannot receive; that would be a configuration gap, **not** Domain Rule 4's `unreached`, which means a fallback chain that was tried and ran out.
...
**ASL / video channel** (`delivery_service._send_video`, `_clip_addresses`, `services/asl_clips.py`, #19):
- A household on `video` or `whatsapp` is sent **one `messages.create_async` carrying both a clip and a caption**: `media_url=[clip_url]` is the pre-recorded interpreter clip, and the body is `AlertContent.video_caption_text`. Mechanically it is the SMS path with media attached, which is the point — the same `statusCallback`, the same refused-send handling, the same reroute trigger, so nothing below the send needed building for this channel.
- `video` and `whatsapp` are **one send with two names**. Which Twilio product carries it is configuration's call, not the household's wording: WhatsApp when `TWILIO_WHATSAPP_NUMBER` is set (both addresses prefixed `whatsapp:`, normalised so a number already written with the scheme is not doubled), MMS from `TWILIO_PHONE_NUMBER` otherwise. MMS is the fallback transport, not a lesser channel — same clip, same caption, addressed differently.
- **The clip is chosen, not generated** (see "Decision: ASL delivery"). `asl_clips.select_clip` keys a versioned manifest on `(severity, action_tag)`, where `action_tag` ∈ {`evacuate`, `shelter_in_place`, `boil_water`, `avoid_area`, `general`} is matched from the dispatcher's own English `alert.title` + `alert.raw_message` by an ordered keyword table, first match winning and most urgent action first. It is deliberately **not** matched on the generated `asl_video_caption`: the caption is written in the household's language, so two deaf households on one alert would match differently and be sent different clips for the same emergency — and letting generated text pick the video would hand Claude a routing decision Domain Rule 1 keeps on the dispatcher's side.
- **Selection never fails.** It is pure, does no I/O, and falls back to the severity's `general` clip on no keyword match, then to `warning`'s for a severity outside the enum. A deaf household has no channel it can fall back to as easily, so a missing asset is never allowed to be the reason it hears nothing. Which clip went out is logged as `delivery.asl_clip_selected` with `alert_id`, `household_id`, `clip_id` and `action_tag` — logged, not stored: a column on `DeliveryAttempt` would be a migration for something the log already answers.
- **The clip carries the action; the caption carries the facts.** A pre-recorded clip cannot know this alert's shelter address or route, and one that claimed to would be inventing content (Domain Rule 1). So the clip never goes out alone — the generated caption, written at the household's literacy level and one instruction per line, is the same message's body. That the specifics reach an ASL household as text is the accepted cost of the clip library, recorded in the decision note.
- The clips themselves are **not in the repo**: `asl_clips.py` holds filenames and `config.ASL_CLIP_BASE_URL` (with a matching `.env.example` entry) says where they are served from, because Twilio fetches the media over the public internet rather than being handed it. An unset base URL raises out of `clip_url`, which the send's own `except` records as a failed attempt naming `ASL_CLIP_BASE_URL` — so a deployment with no clip library says so on the row (and reroutes the household onto a channel that can still reach it) instead of handing Twilio an unresolvable URL that would read as a carrier problem.
- No model changed and no migration was needed: `Channel.VIDEO`/`Channel.WHATSAPP`, `AlertContent.video_caption_text` and the whole status/idempotency/fallback/console path already existed and are channel-agnostic.
...
ASL_CLIP_BASE_URL= # public base URL the ASL interpreter clips are served from
DATABASE_URL=
